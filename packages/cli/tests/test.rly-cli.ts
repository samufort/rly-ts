...
describe("rly-cli", () => {
    ...
    it("it should estimate reverse swap on tbc", async () => {
        const { payer } = wallet;
        const result = test("tbc-swap-estimate", `${tbcPubKey} ${tokenB.publicKey.toBase58()} ${tokenA.publicKey.toBase58()} ${swapInitAmountTokenA.toNumber()} -k ${process.env.KEYPAIR_DEVNET}`)
        console.log(result);
    });
